...
class TestConfigLoader:
    ...
    def test_load_default_config(self):
        loader = ConfigLoader(
            ...
            custom_config_path=self.empty_config,
        )
        config = loader.default_config
        assert (
            config is not None
        )  # Add more specific checks based on your default config
    ...
    def test_load_custom_config(self):
        ...
        assert (
            config is not None
        )  # Add more specific checks based on your custom config

    def test_missing_required_fields(self):
        loader = ConfigLoader(
            default_config_path="tests/assets/incomplete.yml",
            custom_config_path=self.empty_config,
        )
    ...
    def test_task_validation_logic(self):
        loader = ConfigLoader(
            default_config_path="tests/assets/invalid_task.yml",
            custom_config_path=self.empty_config,
        )
        with pytest.raises(ValueError):
            _ = loader.merged_config
    ...
    def test_file_not_found(self):
        loader = ConfigLoader(
            default_config_path="nonexistent.yml", custom_config_path=self.empty_config
        )
    ...
    def test_config_merging(self):
        loader = ConfigLoader(
            default_config_path=self.default_config,
            custom_config_path=self.custom_config,
        )
